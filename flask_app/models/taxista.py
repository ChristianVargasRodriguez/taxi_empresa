...
class Taxista:
    ...
    @classmethod
    def save(cls,data):
        query = "INSERT INTO taxistas (nombre, apellido, empresa, es_conductor, email, password, created_at, updated_at) VALUES(%(nombre)s, %(apellido)s, %(empresa)s, %(es_conductor)s, %(email)s, %(password)s, NOW(), NOW())"
    ...
    def get_one_taxista(cls,data):
        query = "SELECT * FROM taxistas WHERE id = %(id)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        if len(results) < 1:
            return False
    ...
    def get_by_email(cls,data):
        query = "SELECT * FROM taxistas WHERE email = %(email)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        if len(results) < 1:
            return False
    ...
    def get_by_empresa(cls,data):
        query = "SELECT * FROM taxistas WHERE empresa = %(empresa)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        if len(results) < 1:
            return False
        return Taxista(results[0])
    ...
    def get_by_es_conductor(cls,data):
        query = "SELECT * FROM taxistas WHERE es_conductor = %(es_conductor)s;"
        results = connectToMySQL(cls.db_name).query_db(query,data)
        if len(results) < 1:
            return None
        return Taxista(results[0])
